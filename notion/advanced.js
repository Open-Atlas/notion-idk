...
exports.relationSync = async (id) => { //database id
	...
	pages.forEach(async (page) => {
		...
		try{
			...
			!databaseProperties.Mentions ? await notion.updateDatabase(id, {"properties":{"Mentions":{"rich_text":{}}}}) : ''
		} catch(e){
	...
	pages.forEach((page) => {
		page.properties.Software.relation.forEach(s => {
			page.properties.Country.relation.forEach(c => {
				...
				softwareList[s][c] = softwareList[s][c] && softwareList[s][c].length ? [...softwareList[s][c], formatPage(page)] : [formatPage(page)]
			})
		})
	})
